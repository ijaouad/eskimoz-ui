...
export interface TypeInputButton extends ButtonProps {
...
export const AntButton = ({ children, size, icon, disabled, variant, dropdown, ...props }: TypeInputButton) => {

    return (
        <>
            {/* <button {...props} className={`button button-${variant} button-${size}`} disabled={disabled}></button> */}
            <Button type="primary" icon={icon && <PlusIcon />} size={size} className={`button button-${variant} button-${size}`} disabled={disabled}>
                {children}
                { dropdown ?
